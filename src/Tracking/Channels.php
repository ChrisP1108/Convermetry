 ...
 *    name (extendable via the 'convermetry_source_aliases' filter), and
 ...
 *    the 'convermetry_channel' filter).
 *
 * This is the ONLY attribution engine in the plugin: the analytics dashboard,
 * the analytics webhook payloads, and every form submission's
 * analytics_context.channel all classify through this class, so the numbers
 * can never disagree.
 *
...
final class Channels
{
    ...
    public static function normalizeSource(string $source): string
    {
         ...
         */
        $aliases = (array) apply_filters('convermetry_source_aliases', $aliases);
    ...
    public static function classify(array $row, string $type): string
    {
         ...
         */
        return (string) apply_filters('convermetry_channel', $channel, $row, $type);
    }
