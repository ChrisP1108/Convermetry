...
final class DateRangeTest extends TestCase
{
    ...
    {
        // Not yet implemented on main — see plan Phase 1f (finding 8). The guard
        // removes itself: the moment parseDateParam() exists, these run.
        if (!method_exists(DeliveryLogController::class, 'parseDateParam')) {
            self::markTestSkipped(
                'Phase 1f (finding 8): DeliveryLogController::parseDateParam() is not implemented on main. '
                . "'before' is registered but never read, and 'after' collapses to a year+month filter."
            );
        }
