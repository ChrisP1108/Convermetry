...
final class ElementorIdentityTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Not yet implemented on main — see plan Phase 1e (finding 4). Elementor
        // still keys per-form settings by form NAME, so two widgets sharing a
        // name share settings. The guard removes itself once resolveKey() lands.
        if (!method_exists(FormSettings::class, 'resolveKey')) {
            self::markTestSkipped(
                'Phase 1e (finding 4): FormSettings::resolveKey() is not implemented on main. '
                . 'Elementor keys settings by form name rather than a stable widget identity.'
            );
        }
