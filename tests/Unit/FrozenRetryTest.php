...
final class FrozenRetryTest extends TestCase
{
    ...
    /**
     * Skips a test that needs the signing secret resolved by a durable endpoint
     * id. On main it is resolved by URL — Options::secretFor($endpointUrl) — so
     * the 'endpoint-uuid-1' these tests pass matches no endpoint and silently
     * falls through to the SHARED secret. See plan Phase 1a (finding 10).
     */
    private function requiresIdBasedSecretLookup(): void
    {
        if (!method_exists(Options::class, 'ensureEndpointIds')) {
            self::markTestSkipped(
                'Phase 1a (finding 10): the signing secret is resolved by URL, not by a durable '
                . 'endpoint id, so a renamed or deleted endpoint falls back to the shared secret.'
            );
        }
    }
    ...
    {
        $this->requiresIdBasedSecretLookup();
    ...
    {
        // This one PASSES on main, but vacuously: both signatures fall through to
        // the shared secret, so they match for the wrong reason. Skipping it is
        // more honest than banking a green tick that asserts nothing.
        $this->requiresIdBasedSecretLookup();
